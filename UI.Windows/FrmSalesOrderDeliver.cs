...
namespace UI.Windows
{
    ...
    /// Developed by Nuwan
    /// </summary>
    public partial class FrmSalesOrderDeliver : UI.Windows.FrmBaseTransactionForm
    {
        ...
        private string GetDocumentNo(bool isTemporytNo,string type)
        {
            try
            {
                ...
                {
                    return salesOrderService.GetDocumentNo("FrmSalesOrder".ToString() + "V", Common.ConvertStringToInt(cmbLocation.SelectedValue.ToString()), locationService.GetLocationsByID(Common.ConvertStringToInt(cmbLocation.SelectedValue.ToString())).LocationCode, documentID, isTemporytNo).Trim();
                }
                else
                {
                    return salesOrderService.GetDocumentNo("FrmSalesOrder", Common.ConvertStringToInt(cmbLocation.SelectedValue.ToString()), locationService.GetLocationsByID(Common.ConvertStringToInt(cmbLocation.SelectedValue.ToString())).LocationCode, documentID, isTemporytNo).Trim();
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateGrid(SalesOrderDetailTemp salesOrderDetailTemp)
        {
            try
            {
                ...
                //if (((Common.ConvertStringToDecimalQty(txtQty.Text.Trim()) + Common.ConvertStringToDecimalQty(txtSize.Text.Trim())) > 0) && (Common.ConvertStringToDecimalCurrency(txtRate.Text.Trim()) > 0))
                //{
                //    salesOrderDetailTemp.UnitOfMeasureID = Common.ConvertStringToLong(cmbUnit.SelectedValue.ToString());
                //    salesOrderDetailTemp.UnitOfMeasure = cmbUnit.Text.ToString().Trim();
                //    salesOrderDetailTemp.BaseUnitID = existingInvProductMaster.UnitOfMeasureID;
                ...
                //    else
                //    {
                //        qty = (salesOrderDetailTemp.OrderQty + Common.ConvertStringToDecimalQty(txtQty.Text));
                //        freeQty = (salesOrderDetailTemp.FreeQty + Common.ConvertStringToDecimalQty(txtSize.Text));
                //    }

                //    if (!chkOverwrite.Checked)
                //    {
                //        CalculateLine(qty);
                //        salesOrderDetailTemp.OrderQty = Common.ConvertDecimalToDecimalQty(qty);
                //        salesOrderDetailTemp.FreeQty = Common.ConvertDecimalToDecimalQty(freeQty);
                //        salesOrderDetailTemp.GrossAmount = (Common.ConvertDecimalToDecimalQty(qty) * Common.ConvertStringToDecimalCurrency(txtRate.Text.Trim()));
                //    }
                //    else
                //    {
                //        CalculateLine();
                //        salesOrderDetailTemp.OrderQty = Common.ConvertStringToDecimalQty(txtQty.Text);
                //        salesOrderDetailTemp.FreeQty = Common.ConvertStringToDecimalQty(txtSize.Text);
                //        salesOrderDetailTemp.GrossAmount = (Common.ConvertStringToDecimalCurrency(txtQty.Text.Trim()) * Common.ConvertStringToDecimalCurrency(txtRate.Text.Trim()));
                //    }

                //    salesOrderDetailTemp.CostPrice = Common.ConvertStringToDecimalCurrency(txtRate.Text);
                //    salesOrderDetailTemp.DiscountPercentage = Common.ConvertStringToDecimalCurrency(txtGauge.Text.Trim());
                //    salesOrderDetailTemp.DiscountAmount = Common.ConvertStringToDecimalCurrency(txtProductDiscountAmount.Text);
                //    salesOrderDetailTemp.NetAmount = Common.ConvertStringToDecimalCurrency(txtProductAmount.Text);
        ...
        public override void Save()
        {
            ...
            {
                bool saveDocument =   true;// SaveDocument(1, txtDocumentNo.Text.Trim());
                
                    foreach (DataGridViewRow row in dgvItemDetails.Rows)
                    {
                        if (dgvItemDetails["ProductCode", row.Index].Value.ToString()!= string.Empty )
                        {
                            bool isdeliver = Common.ConvertStringToBool(dgvItemDetails["IsDelete", row.Index].Value.ToString());
                            int isdeliverint = isdeliver== true? 1 :0;

                            string sqlst = "update sd set IsDelete = " + isdeliverint + " from SalesOrderDetail sd inner join SalesOrderHeader sh on sh.SalesOrderHeaderID = sd.SalesOrderHeaderID where sd.ProductCode = '" + dgvItemDetails["ProductCode", row.Index].Value.ToString() + "' and sd.[LineNo] = " + Common.ConvertStringToLong(dgvItemDetails["Row", row.Index].Value.ToString()) + " and sh.DocumentNo = '"+ txtDocumentNo.Text.Trim() +"' ";
        ...
        private bool SaveDocument(int documentStatus, string documentNo)
        {
            try
            {
                ...
                //salesOrderHeader.OtherCharges = Common.ConvertStringToDecimalCurrency(txtOtherCharges.Text.ToString());
