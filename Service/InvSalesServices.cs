...
namespace Service
{
    ...
    {        
        ...
        public InvSalesHeader2 getSalesHeader2ByDocumentNo(int documentID, string documentNo, int locationID)
        {
            using (ERPDbContext context = new ERPDbContext())
            { return context.InvSalesHeader2s.Where(ph => ph.DocumentNo.Equals(documentNo)).FirstOrDefault();}
        }

        public string getLastReDispatchNo(string dispatchNo,int locationID)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                var lastDoc = (from a in context.InvSalesHeader2s
                               where a.DocumentNo2.Equals(dispatchNo)
                               orderby a.InvSalesHeader2ID descending
                               select new
                                {
                                    a.DocumentNo,
                                    a.DocumentNo2
                ...
                if (lastDoc != null )
                {
                    string incno = lastDoc.DocumentNo.ToString().Substring((lastDoc.DocumentNo.Length) - 1, 1);


                    switch (incno)
                    {
                        case "A":
                            return lastDoc.DocumentNo2.ToString() + "B";
                            break;
                        case "B":
                            return lastDoc.DocumentNo2.ToString() + "C";
                            break;
                        case "C":
                            return lastDoc.DocumentNo2.ToString() + "D";
                            break;
                        case "D":
                            return lastDoc.DocumentNo2.ToString() + "E";
                            break;
                        case "E":
                            return lastDoc.DocumentNo2.ToString() + "F";
                            break;
                        case "F":
                            return lastDoc.DocumentNo2.ToString() + "G";
                            break;
                        case "G":
                            return lastDoc.DocumentNo2.ToString() + "H";
                            break;
                        case "H":
                            return lastDoc.DocumentNo2.ToString() + "I";
                            break;
                        case "I":
                            return lastDoc.DocumentNo2.ToString() + "J";
                            break;
                        case "J":
                            return lastDoc.DocumentNo2.ToString() + "K";
                            break;
                        case "K":
                            return lastDoc.DocumentNo2.ToString() + "L";
                            break;
                        case "L":
                            return lastDoc.DocumentNo2.ToString() + "M";
                            break;
                        case "M":
                            return lastDoc.DocumentNo2.ToString() + "N";
                            break;
                        case "N":
                            return lastDoc.DocumentNo2.ToString() + "O";
                            break;
                        case "O":
                            return lastDoc.DocumentNo2.ToString() + "P";
                            break;
                        case "P":
                            return lastDoc.DocumentNo2.ToString() + "Q";
                            break;
                        case "Q":
                            return lastDoc.DocumentNo2.ToString() + "R";
                            break;
                        case "R":
                            return lastDoc.DocumentNo2.ToString() + "S";
                            break;
                        case "S":
                            return lastDoc.DocumentNo2.ToString() + "T";
                            break;
                        case "T":
                            return lastDoc.DocumentNo2.ToString() + "U";
                            break;
                        case "U":
                            return lastDoc.DocumentNo2.ToString() + "V";
                            break;
                        case "V":
                            return lastDoc.DocumentNo2.ToString() + "W";
                            break;
                        case "W":
                            return lastDoc.DocumentNo2.ToString() + "X";
                            break;
                        case "X":
                            return lastDoc.DocumentNo2.ToString() + "Y";
                            break;
                        case "Y":
                            return lastDoc.DocumentNo2.ToString() + "Z";
                            break;
                        case "Z":
                            return lastDoc.DocumentNo2.ToString() + "?";
                            break;
                        case "?":
                            return lastDoc.DocumentNo2.ToString() + "?";
                            break;
                        default:
                            return lastDoc.DocumentNo2.ToString() + "A";
                            break;
        ...
        public InvSalesHeader GetSavedSalesHeaderByDocumentID(string docno)
        {
            using (ERPDbContext context = new ERPDbContext())
            { return context.InvSalesHeaders.Where(ph => ph.DocumentNo.Equals(docno) && ph.DocumentStatus.Equals(1)).FirstOrDefault(); }
        }
        ...
        public string[] GetAllReorderDocumentNumbersToInvoice(int documentID, int locationID)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                List<string> documentNoList = new List<string>();

                var sql = (from ph in context.InvSalesHeader2s
                           ...
                           {
                               DocumentNo = poGroup.Key.DocumentNo,
                               IssuedQty = poGroup.Sum(x => x.IssuedQty),
                           }).ToArray();

                foreach (var temp in sql)
                {
                    documentNoList = sql.Where(a => a.IssuedQty > 0).Select(a => a.DocumentNo).ToList();
                }
        ...
        public bool Save(InvSalesHeader2 invSalesHeader2, List<InvSalesDetailTemp2> invSalesDetail2Temp, List<InvProductSerialNoTemp> invProductSerialNoTemp, out string newDocumentNo, List<OtherExpenseTemp> otherExpenceTempList, List<PaymentTemp> paymentTempList, decimal paidAmount, decimal exchangeRate, string formName = "", bool isTStatus = false)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    newDocumentNo = invSalesHeader2.DocumentNo;

                    context.Configuration.AutoDetectChangesEnabled = false;
                    context.Configuration.ValidateOnSaveEnabled = false;

                    if (invSalesHeader2.InvSalesHeader2ID.Equals(0))
                        context.InvSalesHeader2s.Add(invSalesHeader2);
                    else
                        context.Entry(invSalesHeader2).State = EntityState.Modified;
                    ...
                    context.Set<InvSalesDetail2>().Delete(context.InvSalesDetail2s.Where(p => p.InvSalesHeader2ID.Equals(invSalesHeader2.InvSalesHeader2ID) && p.LocationID.Equals(invSalesHeader2.LocationID) && p.DocumentID.Equals(invSalesHeader2.DocumentID)).AsQueryable());
                    context.SaveChanges();


                    var invSalesDetailSaveQuery = (from pd in invSalesDetail2Temp
                                                   where pd.IssuedQty>0
                                                   select new
                                                   {
                                                       InvSalesDetail2ID = 0,
                                                       SalesDetail2ID = 0,
                                                       InvSalesHeader2ID = invSalesHeader2.InvSalesHeader2ID,
                                                       CompanyID = invSalesHeader2.CompanyID,
                                                       LocationID = invSalesHeader2.LocationID,
                                                       CostCentreID = invSalesHeader2.CostCentreID,
                                                       DocumentID = invSalesHeader2.DocumentID,
                                                       LineNo = pd.LineNo,
                                                       ProductID = pd.ProductID,
                                                       Descrip = string.Empty,
                                                       BarCodeFull = 0,
                                                       ...
                                                       IDiscount5 = 0,
                                                       IDI5CashierID = 0,
                                                       GrossAmount = pd.IssuedQty * pd.Rate,///pd.GrossAmount,
                                                       IsSDis = false,
                                                       SDNo = 0,
                                                       SDID = 0,
                                                       SDIs = 0,
                                                       ...
                                                       SalesmanID = 0,
                                                       CashierID = 0,
                                                       StartTime = invSalesHeader2.StartTime,
                                                       EndTime = invSalesHeader2.EndTime,
                                                       UnitNo = 0,
                                                       ShiftNo = 0,
                                                       IsRecall = false,
                                                       RecallNo = 0,
                                                       RecallAdv = false,
                                                       TaxAmount = 0,
                                                       IsTax = (invSalesHeader2.TaxAmount > 0 ? true : false),
                                                       TaxPercentage = 0,
                                                       IsStock = false,
                                                       UpdateBy = 0,
                                                       Status = 1,
                                                       ...
                                                       TaxAmount4 = pd.TaxAmount4,
                                                       TaxAmount5 = pd.TaxAmount5,
                                                       NetAmount = pd.IssuedQty * pd.Rate,
                                                       DispatchLocationID = pd.DispatchLocationID,
                                                       CurrencyID = 0,
                                                       CurrencyAmount = 0,
                                                       CurrencyRate = 0,
                                                       IsUpLoad = invSalesHeader2.IsUpLoad,
                                                       DocumentStatus = invSalesHeader2.DocumentStatus,
                                                       GroupOfCompanyID = invSalesHeader2.GroupOfCompanyID,
                                                       CreatedUser = invSalesHeader2.CreatedUser,
                                                       CreatedDate = invSalesHeader2.CreatedDate,
                                                       ModifiedUser = invSalesHeader2.ModifiedUser,
                                                       ModifiedDate = invSalesHeader2.ModifiedDate,
                                                       DataTransfer = invSalesHeader2.DataTransfer,
                                                       pd.ProductCode,
                                                       pd.ProductName,
                                                       pd.Size,
                                                       pd.Gauge
                    ...
                    string mergeSql1 = CommonService.MergeSqlToForTrans(invSalesDetailSaveQuery.ToDataTable(), "InvSalesDetail2");
                    CommonService.BulkInsert(invSalesDetailSaveQuery.ToDataTable(), "InvSalesDetail2", mergeSql1);
                    ...
                    if (invSalesHeader2.DocumentStatus.Equals(1))
                    {
                        //context.InvProductStockMasters.Update(ps => context.InvPurchaseDetails.Any(pd=> ps.LocationID.Equals(invPurchaseHeader.LocationID) && ps.ProductID.Equals(pd.ProductID), ps => new InvProductStockMaster { Stock = ps.Stock + (pd.Qty * ps.ConvertFactor) });
                    ...
                    var parameter = new DbParameter[] 
                        { 
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@InvSalesHeaderID", Value=invSalesHeader2.InvSalesHeader2ID},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@LocationID", Value=invSalesHeader2.LocationID},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@DocumentID", Value=invSalesHeader2.DocumentID},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@InvReferenceDocumentHeaderID", Value=invSalesHeader2.ReferenceDocumentID},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@InvReferenceDocumentDocumentID", Value=invSalesHeader2.ReferenceDocumentDocumentID},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@RecallDocumentStatus", Value=invSalesHeader2.RecallDocumentStatus},
                            new System.Data.SqlClient.SqlParameter { ParameterName ="@IsDispatch", Value=invSalesHeader2.IsDispatch},

                            new System.Data.SqlClient.SqlParameter { ParameterName ="@DocumentStatus", Value=invSalesHeader2.DocumentStatus}
                        };


                    if (CommonService.ExecuteStoredProcedure("spInvInvoiceReDispatchSave", parameter))
                    {
                        transaction.Complete();
                        return true;
                    }
        ...
        public List<InvSalesDetailTemp> GetSalesOrderDetail(SalesOrderHeader salesOrderHeader)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                ...
                foreach (var tempProduct in salesOrderDetailTemp)
                {
                    ...
                    invSalesDetailTemp.DiscountAmount = tempProduct.DiscountAmount;
                    invSalesDetailTemp.DiscountPercentage = tempProduct.DiscountPercentage;
                    invSalesDetailTemp.GrossAmount = 0; // tempProduct.GrossAmount;
                    invSalesDetailTemp.LineNo = tempProduct.LineNo;
                    invSalesDetailTemp.LocationID = tempProduct.LocationID;
                    invSalesDetailTemp.NetAmount = 0;  // tempProduct.SellingPrice * tempProduct.BalanceQty;  // tempProduct.NetAmount;
                    invSalesDetailTemp.Qty = tempProduct.OrderQty;
                    invSalesDetailTemp.BalanceQty = tempProduct.BalanceQty;
                    ...
                    invSalesDetailTemp.UnitOfMeasure = tempProduct.UnitOfMeasureName;
                    invSalesDetailTemp.UnitOfMeasureID = tempProduct.UnitOfMeasureID;
                    invSalesDetailTemp.Size = tempProduct.Size;
        ...
        public List<InvSalesDetailTemp2> GetInvoceDetailReDispatch(InvSalesHeader invSalesHeader)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                InvSalesDetailTemp2 invSalesDetailTemp2;
                ...
                foreach (var tempProduct in salesOrderDetailTemp)
                {
                    ...
                    invSalesDetailTemp2 = new InvSalesDetailTemp2();

                    invSalesDetailTemp2.Rate = tempProduct.CostPrice;
                    //invSalesDetailTemp.PackSize = tempProduct.PackSize;
                    //invSalesDetailTemp.CurrentQty = tempProduct.CurrentQty;
                    invSalesDetailTemp2.DiscountAmount = tempProduct.DiscountAmount;
                    invSalesDetailTemp2.DiscountPercentage = tempProduct.DiscountPercentage;
                    invSalesDetailTemp2.GrossAmount = 0;// tempProduct.GrossAmount;
                    invSalesDetailTemp2.LineNo = tempProduct.LineNo;
                    invSalesDetailTemp2.LocationID = tempProduct.LocationID;
                    invSalesDetailTemp2.NetAmount = 0;// tempProduct.NetAmount;
                    invSalesDetailTemp2.Qty = tempProduct.OrderQty;
                    invSalesDetailTemp2.BalanceQty = tempProduct.BalanceQty;
                    invSalesDetailTemp2.DispatchQty = tempProduct.OrderQty - tempProduct.BalanceQty;
                    invSalesDetailTemp2.IssuedQty = 0; //tempProduct.BalanceQty;
                    invSalesDetailTemp2.ProductCode = tempProduct.ProductCode;
                    invSalesDetailTemp2.ProductID = tempProduct.ProductID;
                    invSalesDetailTemp2.ProductName = tempProduct.ProductName;
                    invSalesDetailTemp2.IsBatch = false;
                    invSalesDetailTemp2.Rate = tempProduct.SellingPrice;
                    invSalesDetailTemp2.SubTotalDiscount = tempProduct.SubTotalDiscount;
                    invSalesDetailTemp2.TaxAmount1 = tempProduct.TaxAmount1;
                    invSalesDetailTemp2.TaxAmount2 = tempProduct.TaxAmount2;
                    invSalesDetailTemp2.TaxAmount3 = tempProduct.TaxAmount3;
                    invSalesDetailTemp2.TaxAmount4 = tempProduct.TaxAmount4;
                    invSalesDetailTemp2.TaxAmount5 = tempProduct.TaxAmount5;
                    invSalesDetailTemp2.UnitOfMeasure = tempProduct.UnitOfMeasureName;
                    invSalesDetailTemp2.UnitOfMeasureID = tempProduct.UnitOfMeasureID;

                    invSalesDetailTemp2List.Add(invSalesDetailTemp2);
                }
                return invSalesDetailTemp2List.OrderBy(pd => pd.LineNo).ToList();
            }
        }


        public List<InvSalesDetailTemp2> GetReorderDetail(InvSalesHeader2 invSalesHeader2)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                InvSalesDetailTemp2 invSalesDetailTemp2;
                ...
                var salesOrderDetailTemp = (from qd in context.InvSalesDetails
                                            where qd.LocationID.Equals(invSalesHeader2.LocationID) && qd.CompanyID.Equals(invSalesHeader2.CompanyID) &&
                                            qd.DocumentID.Equals(invSalesHeader2.DocumentID) && qd.InvSalesHeaderID.Equals(invSalesHeader2.InvSalesHeader2ID)
                                            select new
                                            {
                                                qd.ConvertFactor,
                                                qd.CostPrice,
                ...
                foreach (var tempProduct in salesOrderDetailTemp)
                {
                    ...
                    invSalesDetailTemp2.DiscountAmount = tempProduct.DiscountAmount;
                    invSalesDetailTemp2.DiscountPercentage = tempProduct.DiscountPercentage;
                    invSalesDetailTemp2.GrossAmount = tempProduct.GrossAmount;
                    invSalesDetailTemp2.LineNo = tempProduct.LineNo;
                    invSalesDetailTemp2.LocationID = tempProduct.LocationID;
                    invSalesDetailTemp2.NetAmount = tempProduct.NetAmount;
                    invSalesDetailTemp2.Qty = tempProduct.OrderQty;
                    invSalesDetailTemp2.BalanceQty = tempProduct.BalanceQty;
                    invSalesDetailTemp2.DispatchQty = tempProduct.OrderQty - tempProduct.BalanceQty;
                    invSalesDetailTemp2.IssuedQty = tempProduct.BalanceQty;
                    invSalesDetailTemp2.ProductCode = tempProduct.ProductCode;
                    invSalesDetailTemp2.ProductID = tempProduct.ProductID;
                    invSalesDetailTemp2.ProductName = tempProduct.ProductName;
                    invSalesDetailTemp2.IsBatch = false;
        ...
        public bool SaveDispatch(InvSalesHeader invSalesHeader, List<InvSalesDetailTemp> invSalesDetailTemp, List<InvProductSerialNoTemp> invProductSerialNoTemp, string OrderNo, decimal exchangeRate, out string newDocumentNo, string invoiceNo, int invoiceID, string formName = "", bool isTStatus = false)
        {
            using (ERPDbContext context = new ERPDbContext())
            {
                using (TransactionScope transaction = new TransactionScope())
