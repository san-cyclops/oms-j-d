    ...
    public class LoyaltyCustomerService
    {
        ...
        public DataTable GetActiveOrdersFilterGrid()
        {
            ...
            this.sqlStatement = " SELECT sh.DocumentNo OrderNo,sh.ReferenceNo ,CAST(sh.DocumentDate AS DATE) DocumentDate,CAST(sh.DeliverDate AS DATE)DeliverDate " +
                                " ,sh.Company , ( CASE WHEN ( sh.salesorderheaderid IN ( SELECT ih.ReferenceDocumentID FROM   InvSalesHeader ih WHERE  ih.DocumentStatus = 1 ) AND (  sd.BalanceQty > 0 ) ) THEN 'Pending' WHEN ( sh.salesorderheaderid IN ( SELECT ih.ReferenceDocumentID FROM   InvSalesHeader ih WHERE  ih.DocumentStatus = 1 ) AND (  sd.BalanceQty =0 ) ) THEN 'Complieted' ELSE 'Active' END ) ActivePending  , " +
                                  " (CASE WHEN sd.DocumentStatus = 4 THEN 'Cancel' ELSE 'Not Cancel' END) Cancel ," +
                                  " c.CustomerName,sp.SalesPersonName,sh.Remark DeliverAddress " +
                                  " ,sd.ProductCode,sd.ProductName,sd.Size,sd.Gauge  " +
                                  " , sd.OrderQty, (sd.BalanceQty ) balanceqty ,sd.OrderQty -sd.BalanceQty AS DispatchQty ,sd.SellingPrice,(sd.SellingPrice*sd.BalanceQty ) Amount" +
                                  " FROM dbo.SalesOrderHeader sh INNER JOIN dbo.SalesOrderDetail sd ON sd.SalesOrderHeaderID=sh.SalesOrderHeaderID  " +
                                  " INNER JOIN dbo.Customer c ON sh.CustomerID = c.CustomerID  " +
                                  " INNER JOIN dbo.InvSalesPerson sp ON sh.InvSalesPersonID = sp.InvSalesPersonID  " +
                                  " INNER JOIN dbo.InvDepartment d ON sh.DepartmentID= d.InvDepartmentID  " +
                                  " INNER JOIN dbo.InvCategory ca ON sh.CategoryID= ca.InvCategoryID  " +
                                  " INNER JOIN dbo.InvSubCategory sc ON sh.SubCategoryID = sc.InvSubCategoryID " +
                                  " INNER JOIN dbo.InvSubCategory2 sc2 ON sh.SubCategory2ID = sc2.InvSubCategory2ID WHERE sd.IsDelete=0 AND sh.DocumentStatus =1 AND sh.IsDelete=0 ";
                ...
                sqlStatement = sqlStatement + " UNION all " +
                                   ...
                                   " SELECT sh.DocumentNo OrderNo, sh.ReferenceNo, " +
                                   " CAST(sh.DocumentDate AS DATE) DocumentDate, " +
                                   " CAST(sh.DocumentDate AS DATE) DeliverDate, oh.Company, " +
                                   "   'ReOrder'  ActivePending, ( CASE WHEN sd.DocumentStatus = 4 THEN 'Cancel' ELSE 'Not Cancel' END ) Cancel, c.CustomerName, " +
                                   " sp.SalesPersonName, sh.Remark DeliverAddress, " +
                                    " sd.ProductCode, sd.ProductName, sd.Size, sd.Gauge, sd.OrderQty, " +
                                   " ( sd.BalanceQty ) balanceqty, " +
                                   " sd.OrderQty - sd.BalanceQty AS DispatchQty,sd.SellingPrice,(sd.SellingPrice*sd.BalanceQty ) Amount  " +
                                   " FROM dbo.InvSalesDetail2 sd INNER JOIN dbo.InvSalesHeader2 sh  " +
                                   " ON sh.InvSalesHeader2ID = sh.InvSalesHeader2ID " +
                                   " INNER JOIN dbo.InvSalesHeader ish ON ish.DocumentNo = sh.DocumentNo2 " +
                                   " INNER JOIN dbo.SalesOrderHeader oh ON ish.ReferenceDocumentID = oh.SalesOrderHeaderID " +
                                   " INNER JOIN dbo.Customer c ON oh.CustomerID = c.CustomerID " +
                                   " INNER JOIN dbo.InvSalesPerson sp ON oh.InvSalesPersonID = sp.InvSalesPersonID " +
                                   " INNER JOIN dbo.InvDepartment d ON oh.DepartmentID = d.InvDepartmentID " +
                                   " INNER JOIN dbo.InvCategory ca ON oh.CategoryID = ca.InvCategoryID " +
                                   " INNER JOIN dbo.InvSubCategory sc ON oh.SubCategoryID = sc.InvSubCategoryID " +
                                   " INNER JOIN dbo.InvSubCategory2 sc2 ON oh.SubCategory2ID = sc2.InvSubCategory2ID " +
                                   "  ORDER BY sh.DocumentNo DESC ";
                //sqlStatement = sqlStatement + " order by sh.DocumentNo desc";
        ...
        public DataTable GetActiveOrdersSalesFilterGrid()
        {
            DataTable dt = new DataTable();
            /*
            this.sqlStatement = "SELECT 'I'+ SUBSTRING(ih.DocumentNo,2,7) InvoiceNo,oh.DocumentNo OrderNo, ih.ReferenceNo , c.CustomerName,sp.SalesPersonName,ih.Remark DeliverAddress, " +
                                "oh.Unit,oh.Company,ih.DocumentDate,oh.DeliverDate, " +
                                "oh.Colour,id.ProductCode,id.ProductName, od.Size,od.Gauge,oh.Unit, id.SellingPrice, " +
                               " id.NetAmount ,(ih.NetAmount*11/100) SvatUSD,(ih.NetAmount*at.ExchangeRate) NetRs,( (ih.NetAmount*at.ExchangeRate) *11/100) SvatRs, "+
                               " id.Qty ,( CASE WHEN id.DocumentStatus = 4 THEN 'Cancel' ELSE 'Not Cancel' END ) Cancel ,   id.Waight , "+
                               " id.IsDeliver FROM    InvSalesHeader ih "+
                               " INNER JOIN dbo.InvSalesDetail id ON ih.InvSalesHeaderID = id.InvSalesHeaderID "+
                               " INNER JOIN SalesOrderHeader oh ON ih.ReferenceDocumentID = oh.SalesOrderHeaderID "+
                               " INNER JOIN dbo.SalesOrderDetail od ON od.SalesOrderDetailID = id.ProductID "+
                               " INNER JOIN dbo.Customer c ON ih.CustomerID = c.CustomerID  "+
                               " INNER JOIN dbo.InvSalesPerson sp ON ih.SalesPersonID = sp.InvSalesPersonID  "+
                               " INNER JOIN dbo.AutoGenerateInfo at ON at.DocumentID = ih.DocumentID AND at.FormName='FrmInvoice' where id.qty>0  ";

            */
            this.sqlStatement = @" SELECT  'I' + SUBSTRING(ih.DocumentNo, 2, 7) InvoiceNo, oh.DocumentNo OrderNo,
                                ih.ReferenceNo, c.CustomerName, sp.SalesPersonName,ih.Remark DeliverAddress, oh.Unit, 
                                oh.Company, ih.DocumentDate,
                                oh.DeliverDate, oh.Colour, id.ProductCode, id.ProductName, od.Size,od.Gauge, oh.Unit,
                                (CASE WHEN ih.OtherChargers=1 THEN id.SellingPrice*oh.DiscountAmount/ih.ExchangeRate ELSE id.SellingPrice END) SellingPrice, 
                                ((CASE WHEN ih.OtherChargers=1 THEN id.SellingPrice*oh.DiscountAmount/ih.ExchangeRate ELSE id.SellingPrice END)* id.Qty) NetAmount, 
                                 ih.ExchangeRate,
                                ( ((CASE WHEN ih.OtherChargers=1 THEN id.SellingPrice*oh.DiscountAmount/ih.ExchangeRate ELSE id.SellingPrice END)* id.Qty) * 11 / 100 ) SvatUSD,
                                ( ((CASE WHEN ih.OtherChargers=1 THEN id.SellingPrice*oh.DiscountAmount/ih.ExchangeRate ELSE id.SellingPrice END)* id.Qty) * ih.ExchangeRate ) NetRs,
                                ( (((CASE WHEN ih.OtherChargers=1 THEN id.SellingPrice*oh.DiscountAmount/ih.ExchangeRate ELSE id.SellingPrice END)* id.Qty) * ih.ExchangeRate ) * 11 / 100 ) SvatRs, id.Qty,
                                ( CASE WHEN id.DocumentStatus = 4 THEN 'Cancel' ELSE 'Not Cancel'END ) Cancel, id.Waight, id.IsDeliver
                                FROM    InvSalesHeader ih
                                INNER JOIN dbo.InvSalesDetail id ON ih.InvSalesHeaderID = id.InvSalesHeaderID
                                INNER JOIN SalesOrderHeader oh ON ih.ReferenceDocumentID = oh.SalesOrderHeaderID
                                INNER JOIN dbo.SalesOrderDetail od ON od.SalesOrderDetailID = id.ProductID
                                INNER JOIN dbo.Customer c ON ih.CustomerID = c.CustomerID
